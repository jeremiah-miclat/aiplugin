 ...
 * askOpenRouterBatch from watcher.js (renamed askAiSingle/askAiBatch here now that the AI
 * backend is provider-configurable, not OpenRouter-only).
 */
public final class AskProcessor {
    ...
    public AskProcessor(Plugin plugin, BotConfig config, Messages messages, KnowledgeBase kb,
                         AiClient ai, ConversationMemory memory, RateLimiter rateLimiter) {
        this.plugin = plugin;
        this.config = config;
    ...
    public CompletableFuture<Void> handleJoin(String player) {
        ...
        return ai.ask(prompt, GREETING_RAW_MAX_LEN, text -> AskParser.parseGreeting(text, ChatBroadcaster.MC_CHAT_LIMIT))
            .thenCompose(reply -> {
                ...
                ChatBroadcaster.sendChat(plugin, player + ": " + messages.joinTip(), config.broadcastReplies());
                return CompletableFuture.<Void>completedFuture(null);
            })
            .exceptionally(e -> {
                plugin.getLogger().log(Level.WARNING, "[ai-companion] error handling join for " + player, e);
                return null;
            });
    }
        ...
        return askAiBatch(batch).thenCompose(parsedList -> {
            if (parsedList != null) {
                ...
                return seq.thenCompose(v -> {
                    ChatBroadcaster.sendPackedReplies(plugin, allLines, config.broadcastReplies());
                    return CompletableFuture.<Void>completedFuture(null);
                });
            }
            plugin.getLogger().warning("[ai-companion] batch reply malformed, falling back to per-question calls");
            CompletableFuture<Void> seq = CompletableFuture.completedFuture(null);
    ...
    public CompletableFuture<Void> handleAskOne(String player, String message) {
        return askAiSingle(player, message)
            ...
            .thenAccept(lines -> ChatBroadcaster.sendPackedReplies(plugin, lines, config.broadcastReplies()))
            .exceptionally(e -> {
                plugin.getLogger().log(Level.WARNING, "[ai-companion] error handling ask for " + player, e);
                List<String> fallback = List.of(player + ": " + messages.askFailureFallback());
                ChatBroadcaster.sendPackedReplies(plugin, fallback, config.broadcastReplies());
                return null;
            });
    }
    ...
    private CompletableFuture<List<AskSubpart>> askAiSingle(String player, String message) {
        ...
        int budget = ChatBroadcaster.MC_CHAT_LIMIT - (player + ": ").length();
        ...
        int maxLen = (ChatBroadcaster.MC_CHAT_LIMIT + 400) * config.maxAskSubparts();
    ...
    private CompletableFuture<List<List<AskSubpart>>> askAiBatch(List<AskQueue.AskJob> batch) {
        ...
        String globalContext = memory.formatGlobalContext();
        int minBudget = ChatBroadcaster.MC_CHAT_LIMIT - batch.stream()
        ...
        int maxLen = (ChatBroadcaster.MC_CHAT_LIMIT + 200) * batch.size() * config.maxAskSubparts();
    ...
    private CompletableFuture<ResolvedSubpart> resolveSubpart(String player, AskSubpart parsed) {
        ...
        String key = rateLimitKey(player);
    ...
    private String rateLimitKey(String player) {
        ...
        return "name:" + player.toLowerCase();
    }
